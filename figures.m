...
% mt.mat, lobo.mat, mc.mat, mn.mat, transect_HS.mat, key.xlsx, taxa1.txt
...
ylabel(h,'Current velocity (cm/s)',FontSize=11);%set(get(gca,'YLabel'),'Rotation',30)
    ax=gca;xlim('tight');g1=xlim;r=g1(1):1:g1(2);ax.XTick=r; datetick('x','mm/dd','keeplimits');
ylabel('Depth (m)');
xlim([738388 738396])
...
figure(12);subplot(2,1,1) 
plot(time(a)-735599,temp(a));datetick('x','mmm');ylabel('Temperature (°C)');hold on;
plot(time(b)-735964,temp(b));datetick('x','mmm');hold on;
plot(time(c)-736329,temp(c));datetick('x','mmm');hold on;
plot(time(d)-736695,temp(d));datetick('x','mmm');hold on;
plot(time(e)-737060,temp(e));datetick('x','mmm');hold on;
plot(time(f)-737435,temp(f));datetick('x','mmm');hold on;
plot(time(g)-737790,temp(g));datetick('x','mmm');hold on;
plot(time(h)-738156,temp(h),'k');datetick('x','mmm');hold on;
plot(time(i1)-738521,temp(i1),'r');datetick('x','mmm');hold on;
legend('2014','2015','2016','2017','2018','2019','2020','2021','2022','Location','best');
xlim([1 365]);ylim([-1 22]);grid on;

figure(12);subplot(2,1,2) 
plot(time(a)-735599,sal(a));datetick('x','mmm');ylabel('Salinity (ppt)');hold on;
plot(time(b)-735964,sal(b));datetick('x','mmm');hold on;
plot(time(c)-736329,sal(c));datetick('x','mmm');hold on;
plot(time(d)-736695,sal(d));datetick('x','mmm');hold on;
plot(time(e)-737060,sal(e));datetick('x','mmm');hold on;
plot(time(f)-737435,sal(f));datetick('x','mmm');hold on;
plot(time(g)-737790,sal(g));datetick('x','mmm');hold on;
plot(time(h)-738156,sal(h),'k');datetick('x','mmm');hold on;
plot(time(i1)-738521,sal(i1),'r');datetick('x','mmm');hold on;
%legend('2014','2015','2016','2017','2018','2019','2020','2021','2022','Location','best');
xlim([1 365]);ylim([20 34]);grid on;
fontname(gcf,"Times New Roman");

%% Figure 13
figure(13);tiledlayout(5,1);
clearvars
% drag in 'Sensor0052-20230208114311.tsv'
...
colormap(mycolormap);
colorbar off;
datetick('x','mmm');xlim([736695 737059]);ylabel('Depth (m)');
...
colormap(mycolormap);
colorbar off;
datetick('x','mmm');xlim([737060 737424]);ylabel('Depth (m)');

clearvars
% drag in 'Sensor0052-20230208114311.tsv'
 f=12466:17442; % 737425.00 2019
...
datetick('x','mmm');xlim([737425 737789]);ylabel('Depth (m)');

clearvars
% drag in 'Sensor0052-20230208114311.tsv'
 h=21199:25489; % 738156.00 2021
...
colormap(mycolormap);
colorbar off;
datetick('x','mmm');xlim([738156 738520]);ylabel('Depth (m)');
...
set(h, 'ylim', [-15 15])
colormap(mycolormap);
ylabel(h,'Residual current velocity (cm/s)','FontSize',12);
datetick('x','mmm');xlim([738521 738885]);ylabel('Depth (m)');
fontname(gcf,"Times New Roman");
...
for i=[2 3 5 6 7 8 9 10 11 12]           
    ...
    figure(17);subplot(3,1,1);
    yyaxis left
    plot(rdate,dia,'b-');ylim([0 500]);hold on;ylabel('Biovolume (cm^3/m^3)')
    yyaxis right
    plot(rdate,din,'r-');ylim([0 150]);hold on;
    ylabel('Biovolume (cm^3/m^3)');datetick('x','mmm');xlim([737789 738156]);

end
subplot(3,1,1);legend('Diatom','Dinoflagellate');
...
for i=[1 2 3 4 6 7 8 9 10 11 12]         
    ...
    plot(rdate,dia,'b-');ylim([0 500]);hold on;ylabel('Biovolume (cm^3/m^3)')
    yyaxis right
    plot(rdate,din,'r-');ylim([0 50]);hold on;
    ylabel('Biovolume (cm^3/m^3)');datetick('x','mmm');xlim([738156 738520]);
...
for i=1:5          
    ...
    yyaxis right
    plot(rdate,din,'r-');ylim([0 50]);hold on;
    ylabel('Biovolume (cm^3/m^3)');datetick('x','mmm');xlim([738521 738886]);
end
...
load mt.mat
roi=10008:39581;
t=mt.timestamp;tn=datenum(t(roi));tn=tn-0.1666666667;% local time
a=mt.images_ml;
load biovolume.mat;
...
hv=C(h(:));lv=C(l(:));
    vhC=interp1(tn(h(:)),hv,tn);
    vlC=interp1(tn(l(:)),lv,tn);
...
hv=t(h(:));lv=t(l(:));
    vht=interp1(tn(h(:)),hv,tn);
    vlt=interp1(tn(l(:)),lv,tn);
    
figure(18)
subplot(3,1,1)
    plot(rdate_intc,Cera,'k-');hold on;
plot(tn,vhC,'b-','LineWidth',2);hold on;
plot(tn,vlC,'r-','LineWidth',2);grid ON;hold on;
    legend('_','High tide population','Low tide population','Location','northwest');
    ylim([0 15]);xlim([738309 738347]);
...
plot(tn,vhc,'b-','LineWidth',2);hold on;
plot(tn,vlc,'r-','LineWidth',2);grid ON;hold on;
ylim([0 95]);xlim([738309 738347]);
   ylabel('Biovolume (cm^3/m^3)'); 
    datetick('x','mmmdd','keeplimits');

    subplot(3,1,3)
      plot(rdate_intc,Tha,'k-');hold on;
plot(tn,vht,'b-','LineWidth',2);hold on;
plot(tn,vlt,'r-','LineWidth',2);grid ON;hold on;
ylim([0 120]);xlim([738309 738347]);
ylabel('Biovolume (cm^3/m^3)');
datetick('x','mmmdd','keeplimits');
 fontname(gcf,"Times New Roman");
...
figure(19);subplot(5,1,1);
colororder({'r','k'});
yyaxis left;plot(time,temp,'r');grid on;
datetick('x','mmm','keeplimits');ylabel('Temperature (°C)');xlim([738491 738652]);set(gca,'Xticklabel',[]);
yyaxis right;
plot(time,sal,'k');grid on;
datetick('x','mmm','keeplimits');ylabel('Salinity (ppt)');xlim([738491 738652]);set(gca,'Xticklabel',[])
...
colorbar('eastoutside');
clim([-12 12]);h = colorbar;set(h, 'ylim', [-15 15]);colormap(mycolormap);
ylabel(h,'Residual current velocity (cm/s)',FontSize=11);
    ax=gca;xlim('tight');g1=xlim;r=g1(1):1:g1(2);ax.XTick=r; datetick('x','mmm','keeplimits');set(gca,'Xticklabel',[]);
ylabel('Depth (m)');
xlim([738491 738652])
...
xlim([738491 738652]);set(gca,'Xticklabel',[]);
...
figure(21)
subplot(3,1,1);area(t,tox_avg);datetick('x','mmm');ylabel('Biovolume (cm^3/m^3)');grid on;%set(gca,'Xticklabel',[]);
...
ylim([-15 15]);xlim([737844 738670]); 

clearvars
% drag in 'Sensor0052-20230208114311.tsv'
h=24757:36339; %2020 through 2022
sensor=table2array(Sensor005220230208114311(h,2:91));
for n=1:30
    U(1,n)=3+(3*(n-1));E(1,n)=1+(3*(n-1));N(1,n)=2+(3*(n-1));x1(1,n)=0;y1(1,n)=n*(-1);end
east=sensor(:,E)/10;%(mm/s to cm/s)
...
set(h, 'ylim', [-15 15])
colormap(mycolormap);
datetick('x','mmm');xlim([737844 738670]);ylabel('Depth (m)');
    fontname(gcf,"Times New Roman");
